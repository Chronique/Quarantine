...
export default function Demo() {
  ...
  // State untuk Saldo dan Penarikan
  ...
  // State untuk Target Swap (Pilihan User)
  const [targetToken, setTargetToken] = useState({
    symbol: "ETH",
    address: "ETH" // 'ETH' akan dihandle 0x sebagai Native ETH
  });
  ...
  const handleQuarantineAll = async () => {
    ...
    alert(`Memproses pemindahan ${tokens.length} koin. Silahkan konfirmasi transaksi di dompet Anda.`);
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      alert(`Penarikan ${withdrawPercentage}% berhasil diajukan!`);
      fetchVaultBalance();
    } catch (err) { 
      console.error("Withdrawal gagal:", err);
      alert("Gagal. Vault butuh saldo ETH untuk proses gas estimasi."); 
    }
  };
        ...
        {activeTab === "actions" && (
          <div className="mt-6 space-y-6">
            <div className="bg-amber-50 border border-amber-200 p-4 rounded-2xl flex items-start gap-3">
              ...
              <div>
                <p className="text-xs font-bold text-amber-800 text-left">Info Gas Vault</p>
                <p className="text-[10px] text-amber-700 leading-relaxed text-left">
            ...
            <div className="bg-slate-900 p-6 rounded-[2rem] shadow-2xl text-white relative overflow-hidden">
              <div className="relative z-10 text-left">
                ...
                <div className="bg-white/5 border border-white/10 p-4 rounded-2xl flex justify-between items-center mb-4">
                  ...
                  <button onClick={() => { navigator.clipboard.writeText(vaultAddress!); alert("Copied!"); }} className="p-2 bg-white/10 rounded-xl transition-all active:scale-95">
                    <Copy width={16} />
                  </button>
                </div>
            ...
            <div className="space-y-4">
              {tokens.length > 0 ? tokens.map((token: any) => (
                <div key={token.address} className="flex justify-between items-center p-4 bg-white rounded-2xl border border-slate-100 shadow-sm transition-all hover:shadow-md">
                  ...
                  <button onClick={() => handleQuarantine(token)} className="bg-slate-900 text-white text-[10px] font-black px-4 py-2.5 rounded-xl uppercase tracking-tighter hover:bg-red-500 transition-colors">
                    Quarantine
                  </button>
                </div>
              )) : !isScanning && (
        ...
        {activeTab === "swap" && (
          <div className="mt-6 space-y-4 text-left">
            <div className="bg-white p-6 rounded-[2rem] shadow-xl border border-slate-100">
                  ...
                  {["ETH", "USDC"].map((sym) => (
                    <button 
                      ...
                      onClick={() => setTargetToken({ 
                        symbol: sym, 
                        address: sym === "ETH" ? "ETH" : "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913" 
                      })}
                      className={`px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all ${targetToken.symbol === sym ? "bg-white text-blue-600 shadow-sm" : "text-slate-500"}`}
                    >
                      {sym}
                    </button>
                  ))}
              ...
              <p className="text-[10px] text-slate-500 mb-6 italic text-left">
                Tukar koin yang ada di Vault menjadi <b>{targetToken.symbol}</b> (Fee: 5%).
              </p>
                ...
                {vaultScanner.tokens
                  ...
                  <div key={token.address} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 transition-all hover:bg-white">
                    ...
                    <button 
                      ...
                      className="flex items-center gap-2 bg-blue-600 text-white text-[10px] font-black px-4 py-2.5 rounded-xl uppercase shadow-md active:scale-95 disabled:bg-slate-300 transition-all"
                    >
                      {isSwapping === token.address ? "Swapping..." : `Swap to ${targetToken.symbol}`} <ArrowRight width={12} />
                    </button>
                  </div>
                ))}
          ...
          <div className="mt-6 space-y-4 text-left">
            <div className="bg-white p-6 rounded-[2rem] shadow-xl border border-slate-100">
              ...
              <div className="grid grid-cols-4 gap-2 mb-4">
                {[25, 50, 75, 100].map((pct) => (
                  <button 
                    ...
                    className={`py-2 rounded-xl text-[10px] font-bold transition-all ${withdrawPercentage === pct ? 'bg-blue-600 text-white shadow-md' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
                  >
                    {pct === 100 ? 'MAX' : `${pct}%`}
                  </button>
                ))}
              ...
              <button 
                ...
                className="w-full flex items-center justify-center gap-2 bg-red-500 text-white font-bold py-4 rounded-2xl shadow-lg active:scale-95 transition-all disabled:bg-slate-200"
              >
                <LogOut width={18} /> Withdraw {withdrawPercentage}%
              </button>

              <div className="mt-8 border-t border-slate-100 pt-6">
                <p className="text-xs font-bold text-slate-400 uppercase mb-4 tracking-widest">Koin di Karantina</p>
