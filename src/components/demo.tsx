...
export default function Demo() {
  ...
  const { connect, connectors } = useConnect(); // Hook untuk koneksi manual jika auto-connect gagal
  ...
  // Target Swap (ETH/USDC)
  const [targetToken, setTargetToken] = useState({
    symbol: "ETH",
    address: "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee" // Native ETH 0x standar
  });

  const fetchVaultBalance = useCallback(async () => {
    // Validasi agar tidak memanggil RPC dengan alamat "undefined"
  ...
  const handleQuarantine = async (token: any) => {
    if (!vaultAddress || !isAddress(vaultAddress) || !address) {
      alert("Vault belum siap. Pastikan dompet terhubung.");
      return;
    }
    try {
      sendTransaction({
  ...
  const handleQuarantineAll = async () => {
    ...
    alert(`Memproses ${tokens.length} koin. Konfirmasi satu per satu di dompet Anda.`);
  ...
  const handleSwap = async (token: any) => {
    ...
    try {
      ...
      console.error(err);
      alert("Swap Gagal. Cek saldo ETH di Vault untuk Gas.");
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      alert(`Penarikan ${withdrawPercentage}% berhasil!`);
      fetchVaultBalance();
    } catch (err) { alert("Gagal. Vault butuh sedikit ETH untuk estimasi gas."); }
  };
  ...
  return (
    <div style={{ marginTop: (frameContext?.context as any)?.client?.safeAreaInsets?.top ?? 0, paddingBottom: '100px' }} className="bg-slate-50 min-h-screen font-sans text-left">
      <div className="w-full max-w-lg mx-auto p-4">
        ...
        {!isConnected && (
          <div className="mt-10 bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-100 text-center">
            <div className="w-20 h-20 bg-blue-50 rounded-3xl flex items-center justify-center mx-auto mb-6">
              <Wallet width={40} className="text-blue-600" />
            </div>
            <h3 className="text-xl font-black text-slate-900 mb-2">Wallet Disconnected</h3>
            <p className="text-slate-500 text-sm mb-8 leading-relaxed">Klik tombol di bawah untuk mengaktifkan Vault dan mulai membersihkan koin sampah Anda.</p>
            <button 
        ...
        {isConnected && (
          <>
            {activeTab === "actions" && (
              <div className="mt-6 space-y-6">
                <div className="bg-amber-50 border border-amber-200 p-4 rounded-2xl flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold text-amber-800">Info Gas Vault</p>
                    <p className="text-[10px] text-amber-700 leading-relaxed text-left">Kirim minimal <b>$3 ETH</b> ke alamat Vault agar fitur Swap dan Withdraw lancar.</p>
                  </div>
                </div>
                  ...
                  <div className="relative z-10 text-left">
                    ...
                    <div className="bg-white/5 border border-white/10 p-4 rounded-2xl flex justify-between items-center mb-4">
                      <div className="truncate mr-4">
                        <p className="font-mono text-xs truncate">{vaultLoading ? "Generating..." : (vaultAddress || "Error Load Address")}</p>
                      </div>
                ...
                {tokens.length > 0 && (
                  <div className="flex justify-between items-center px-2">
                    <p className="text-[10px] font-bold text-slate-500 uppercase">{tokens.length} Koin Terdeteksi</p>
                    <button onClick={handleQuarantineAll} className="flex items-center gap-1 bg-blue-100 text-blue-700 text-[10px] font-black px-3 py-2 rounded-lg hover:bg-blue-200 transition-all">
                ...
                <div className="space-y-4">
                  {tokens.length > 0 ? tokens.map((token: any) => (
                    ...
                    <div className="text-center py-12">
                      <Cart width={48} className="mx-auto text-slate-200 mb-2" />
            ...
            {activeTab === "swap" && (
              <div className="mt-6 space-y-4 text-left">
                <div className="bg-white p-6 rounded-[2rem] shadow-xl border border-slate-100">
                  ...
                  <p className="text-[10px] text-slate-500 mb-6 italic">Tukar koin yang ada di Vault menjadi <b>{targetToken.symbol}</b> (Fee: 5%).</p>
                  <div className="space-y-4">
                    {vaultScanner.tokens.filter((t: any) => t.liquidityUSD > 1).map((token: any) => (
                      <div key={token.address} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100">
                        <div className="flex items-center gap-3 text-left">
                          ...
                          <div><p className="font-bold text-slate-800 text-sm">{token.symbol}</p><p className="text-[9px] text-blue-500 font-bold uppercase">Liquid: ${Number(token.liquidityUSD).toLocaleString()}</p></div>
                        </div>
                    ...
                    {vaultScanner.tokens.filter((t: any) => t.liquidityUSD > 1).length === 0 && <div className="text-center py-12 italic text-xs text-slate-400">Pindahkan koin yang memiliki likuiditas ke Vault untuk menukar.</div>}
                  </div>
                </div>
              </div>
            )}
            ...
            {activeTab === "wallet" && (
              <div className="mt-6 space-y-4 text-left">
                <div className="bg-white p-6 rounded-[2rem] shadow-xl border border-slate-100">
                  <div className="mb-6"><h3 className="font-bold text-slate-800 flex items-center gap-2 mb-2"><Wallet width={20} /> Saldo Vault (Base)</h3><div className="text-3xl font-black text-slate-900">{Number(vaultEthBalance).toFixed(6)} <span className="text-sm font-medium text-slate-400">ETH</span></div></div>
                  ...
                  <button onClick={handleWithdraw} disabled={Number(vaultEthBalance) === 0} className="w-full flex items-center justify-center gap-2 bg-red-500 text-white font-bold py-4 rounded-2xl shadow-lg active:scale-95 transition-all disabled:bg-slate-200"><LogOut width={18} /> Withdraw {withdrawPercentage}%</button>
                    ...
                    <div className="space-y-3">
                      {vaultScanner.tokens.length > 0 ? vaultScanner.tokens.map((token: any) => (
                        <div key={token.address} className="flex justify-between items-center p-3 bg-slate-50 rounded-xl border border-slate-100">
                          ...
                          <p className="text-[9px] font-mono text-slate-400">In Vault</p>
                        </div>
