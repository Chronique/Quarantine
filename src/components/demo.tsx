...
import { 
  ...
  WarningTriangle, LogOut, RefreshDouble, ArrowRight, Cart
} from "iconoir-react";
...
export default function Demo() {
  ...
  const handleQuarantineAll = async () => {
    ...
    alert(`Memproses pemindahan ${tokens.length} koin. Silahkan konfirmasi di dompet Anda.`);
          ...
          <div className="mt-6 space-y-6">
            ...
            <div className="bg-slate-900 p-6 rounded-[2rem] shadow-2xl text-white relative overflow-hidden">
              <div className="relative z-10 text-left">
                ...
                <div className="bg-white/5 border border-white/10 p-4 rounded-2xl flex justify-between items-center mb-4 text-left">
            ...
            <div className="space-y-4">
              {tokens.length > 0 ? tokens.map((token: any) => (
                ...
                <div className="text-center py-12 text-slate-400 text-sm italic">Klik scan untuk mencari koin debu.</div>
              )}
            </div>
          </div>
        )}
              ...
              <div className="space-y-4">
                {vaultScanner.tokens.filter((t: any) => t.liquidityUSD > 1).map((token: any) => (
                  <div key={token.address} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    ...
                    <button onClick={() => handleSwap(token)} disabled={isSwapping === token.address} className="flex items-center gap-2 bg-blue-600 text-white text-[10px] font-black px-4 py-2.5 rounded-xl uppercase transition-all">{isSwapping === token.address ? "..." : `Swap to ${targetToken.symbol}`} <ArrowRight width={12} /></button>
                  </div>
                ))}
              </div>
            </div>
        ...
        {activeTab === "wallet" && (
          <div className="mt-6 space-y-4 text-left">
            <div className="bg-white p-6 rounded-[2rem] shadow-xl border border-slate-100">
              ...
              <button onClick={handleWithdraw} disabled={Number(vaultEthBalance) === 0} className="w-full bg-red-500 text-white font-bold py-4 rounded-2xl shadow-lg active:scale-95 transition-all">Withdraw {withdrawPercentage}%</button>
            </div>
          </div>
        )}
